...
Pitch (피치): 고개를 위/아래로 끄덕이는 각도

위로 젖히면 +, 아래로 숙이면 - (프로젝트 설정에 따라 부호는 반대일 수 있음)

Yaw (요): 고개를 좌/우로 “도리도리” 하는 각도

오른쪽을 보면 +, 왼쪽을 보면 - (마찬가지로 부호는 yawSign 같은 설정에 따라 바뀔 수 있음)

Roll (롤): 고개를 좌/우로 기울이는 각도 (귀가 어깨 쪽으로 가까워지는 느낌)

오른쪽으로 기울이면 +, 왼쪽으로 기울이면 - (부호 규칙은 구현에 따라 다름)